...
# 학습과 예측
mlp_sgd.compile(lose="MSE", optimizer=SGD(learning_rate=0.01),metrics=['accuracy'])
hist_sgd=mlp_sgd.fit(x_train,y_train,batch_size=128,epochs=50,validation_data=(x_test,y_teset),verbose=2)
...
# 학습과 예측
mlp_adam.compile(lose="MSE", optimizer=Adam(learning_rate=0.001),metrics=['accuracy'])
hist_adam=mlp_adam.fit(x_train,y_train,batch_size=128,epochs=50,validation_data=(x_test,y_teset),verbose=2)
